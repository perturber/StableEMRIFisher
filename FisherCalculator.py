...
class Fisher:
    ...
    def __init__(self, M, mu, a, p0, e0, Y0, dist, qS, phiS, qK, phiK,\
        ...
        inspiral_kwargs = {
            "DENSE_STEPPING": 1,  # !!! SPARSE STEPPING MAY BE UNSTABLE FOR FISHER CALC. !!!
            "max_init_len": int(1e4),  # Adjust according to dense step traj length
        }
    ...
    def FisherCalc(self):
        ...
        if self.filename == '':
            ...
            np.savetxt('Fisher_'+self.filename+'.txt',Fisher)
    ...
    def CovEllipsePlot(self, covariance):
        ...
        if self.filename == '':
            ...
            plt.savefig('CovEllipse_'+self.filename+'.png',dpi=300,bbox_inches='tight')
