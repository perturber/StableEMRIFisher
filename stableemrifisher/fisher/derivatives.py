...
stencils = {
    ...
    "forward": {
        ...
        ),  # Fornberg 1988: https://doi.org/10.1090%2FS0025-5718-1988-0935077-0
    },
    "backward": {
...
def derivative(
    *args,  # To fool fastlisaresponse
    ...
    if waveform is None:
        ...
        waveform = xp.asarray(
            waveform_generator(*list(parameters.values()), **waveform_kwargs)
        )
    ...
    if param_to_vary == "dist":
        ...
        if kind == "central":
            ...
            for _ in range(order // 2):
                ...
                waveform_delta = xp.asarray(
                    waveform_generator(*temp_vals, **waveform_kwargs)
                )
        ...
        try:
            derivative = (
                xp.asarray(stencils[kind][order])[:, None, None]
                * xp.asarray(delta_waveforms)
            ).sum(0) / delta
        except KeyError:
